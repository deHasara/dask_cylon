...
def read_csv(path, chunksize="256 MiB", **kwargs):
    if "://" in str(path):
        ...
        func = make_reader(pycylon.read_csv, "read_csv", "CSV")
...
def _internal_read_csv(path, chunksize="256 MiB", **kwargs):
    ...
    dask_reader = make_reader(pycylon.read_csv, "read_csv", "CSV")
    print(dask_reader)
...
def _read_csv(fn, dtypes=None, **kwargs):
    return pycylon.read_csv(fn, **kwargs)
...
def read_csv_without_chunksize(path, **kwargs):
    ...
    meta = pycylon.read_csv(filenames[0], nrows=5, **kwargs).iloc[:0]

    graph = {
        (name, i): (apply, pycylon.read_csv, [fn], kwargs)
